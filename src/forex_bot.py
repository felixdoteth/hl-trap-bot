#!/usr/bin/env python3
"""
Price Action Trap Bot — Forex/CFD via MT5 (FBS demo)
Same signal_engine.py, unchanged. Data feed and execution replaced:
Hyperliquid/CoreWriter -> MT5 bridge. SL/TP are native (broker-managed)
instead of the original's client-side polling thread.
"""
...
# ── Config ───────────────────────────────────────────────────────────────
RISK_PCT     = float(os.getenv('RISK_PCT', '0.05'))   # NOTE: unresolved vs original's
                                                        # "0.5%" comment — same default
                                                        # preserved, not changed silently
SL_ATR_MULT  = float(os.getenv('SL_ATR_MULT', '1.5'))
TP_ATR_MULT  = float(os.getenv('TP_ATR_MULT', '5'))
DISCORD_WEBHOOK  = os.getenv('DISCORD_WEBHOOK', '')
TELEGRAM_TOKEN   = os.getenv('TELEGRAM_TOKEN', '')
TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID', '')
DB_PATH      = os.path.expanduser('~/forex_trades.db')
MT5_HOST     = os.getenv('MT5_HOST', 'localhost')
MT5_PORT     = int(os.getenv('MT5_PORT', '18001'))
MAGIC        = 234000
...
def calc_size(symbol, atr, direction):
    """Forex/CFD lot sizing, currency-general. Uses MT5's own
    order_calc_profit() (broker's live conversion rates) instead of
    assuming quote currency == USD, which is what broke USDJPY —
    correct for any pair now, not just direct USD-quoted ones."""
...
def place_order(symbol, direction, lot, entry, sl, tp):
    """Native SL/TP attached to the order — broker enforces it even if
    this process goes down, unlike the original's client-side polling."""
    request = {
        ...
        "deviation": 20,
        "magic": MAGIC,
        "comment": "hl-trap-bot forex",
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_FOK,
    }
    return mt5.order_send(request)

active_positions = {}   # asset -> {ticket, tid, direction, entry, size, trap_type}

last_closed_bar_ts = {}  # asset -> last closed bar ts (to avoid double-processing same bar)
...
def _record_close(asset, tid, ticket, direction, entry, trap_type):
    """Called once a tracked position no longer appears in positions_get() —
    looks up the real closing deal for authoritative price/P&L rather than
    recomputing it from polled price the way the original did."""
...
def check_closed_positions():
    """Poll open MT5 positions once per loop; anything we're tracking that's
    no longer there was closed by the broker (SL or TP hit)."""
...
def reconcile_open_trades():
    """On startup: anything still marked open in the DB — ask the broker
    directly whether it's still open (native SL/TP means the broker may
    have already closed it while this process was down)."""
    c = sqlite3.connect(DB_PATH)
    rows = c.execute(
...
def run():
    ...
    print(f"Forex Trap Bot | Assets: {list(ASSETS.keys())} | Risk: {RISK_PCT*100}%")
    notify(f'**Forex Trap Bot Started**\nAssets: {list(ASSETS.keys())}\nRisk: {RISK_PCT*100}%')
    ...
    while True:
        try:
            ...
            for asset, cfg in ASSETS.items():
                ...
                # Need at least 2 bars: forming + one closed
                if len(df_raw) < 2:
                    print(f"[{now}] {asset}: not enough candles")
                    continue

                # Last row = forming bar; everything before = closed bars only
                ...
                if not result.should_trade:
                    ...
                    last_closed_bar_ts[asset] = closed_ts  # mark bar as seen even on no-trade
                    continue
                ...
                # ATR from the same closed-bar series used for the signal
                ...
                if res.retcode != mt5.TRADE_RETCODE_DONE:
                    ...
                    # Do NOT mark bar processed — allow retry next loop if appropriate
                    continue

                print(f"   Order placed: ticket={res.order}")
                last_closed_bar_ts[asset] = closed_ts  # commit only after successful order path
